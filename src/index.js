...
function chooseHole(holes) {
  ...
  // If the hole is the same as the last hole, call chooseHole recursively
  if (hole === lastHole) {
    return chooseHole(holes);
  }
  
  // Update the lastHole and return the selected hole
  lastHole = hole;
  return hole;
}
...
function gameOver() {
  if (time > 0) {
    // If time is greater than 0, continue the game by calling showUp()
    const timeoutId = showUp();
    return timeoutId;
  } else {
    // If time is 0 or less, stop the game by calling stopGame()
    stopGame();
    return "game stopped";
  }
}
...
*/
function showUp() {
  // Update to use setDelay() to determine the delay
  let delay = setDelay(difficulty); // Update to use setDelay() with the difficulty parameter
  const hole = chooseHole(holes); // Update to use chooseHole() to select a random hole
...
function showAndHide(hole, delay){
  // Call the toggleVisibility function to show the mole by adding the 'show' class
  toggleVisibility(hole);

  const timeoutID = setTimeout(() => {
    // Call the toggleVisibility function to hide the mole by removing the 'show' class when the timer times out
    toggleVisibility(hole);
    gameOver();
